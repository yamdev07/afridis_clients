  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] text-slate-900 dark:text-white transition-colors duration-500 font-sans selection:bg-primary/20">
        ...
        <div className="grid lg:grid-cols-2 gap-24 items-start">
          ...
          <section className="space-y-16">
            <header>
              ...
              <h1 className="text-6xl lg:text-7xl font-black text-slate-900 dark:text-white mb-10 tracking-tight leading-[1.05]">
                Votre projet <br />
          ...
          <section>
            <GlassCard className="p-10 lg:p-14 border-slate-200 dark:border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] dark:shadow-none relative overflow-hidden rounded-[40px]" hover={true}>
              {/* Animated accent bar */}
                ...
                {sent ? (
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Signal Envoyé !</h3>
                    ...
                    <Button variant="secondary" className="!bg-slate-100 dark:!bg-white/5 !text-slate-600 dark:!text-slate-400 font-black uppercase tracking-widest text-[10px]" onClick={() => setSent(false)}>Envoyer un nouveau message</Button>
                  </motion.div>
                ) : (
                  <form key="form" onSubmit={handleSubmit} className="space-y-10">
                    <div className="space-y-8">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Formulaire de Contact</h3>
                      </div>
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1">Sujet de la transmission</label>
                        <div className="relative">
                          <select className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[20px] text-xs font-black text-slate-900 dark:text-white focus:bg-white dark:focus:bg-white/10 focus:ring-8 ring-primary/5 focus:border-primary outline-none transition-all appearance-none cursor-pointer uppercase tracking-[0.15em]">
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1">Détails de la demande</label>
                        <textarea
                          rows="6"
                          placeholder="Décrivez votre besoin avec précision..."
                          className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[20px] text-xs font-bold text-slate-900 dark:text-white focus:bg-white dark:focus:bg-white/10 focus:ring-8 ring-primary/5 focus:border-primary outline-none transition-all resize-none placeholder:text-slate-300 dark:placeholder:text-slate-700"
                          required
                        />
                      </div>
                    </div>
      ...
      <footer className="py-20 border-t border-slate-100 dark:border-white/5 text-center bg-slate-50/50 dark:bg-white/[0.01]">
        ...
        <p className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">© 2026 CLIENTFLOW CRM. UNIVERSE OF EFFICIENCY.</p>
      </footer>
    </div>
  );
}
...
function ContactInfoCard({ icon: Icon, title, value, desc, color }) {
  return (
    <GlassCard className="p-8 border-slate-100 dark:border-white/5 group relative overflow-hidden" hover={true}>
      ...
      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-2 leading-none">{title}</h4>
      <p className="text-lg font-black text-slate-900 dark:text-white mb-2 tracking-tight">{value}</p>
      <p className="text-[10px] font-bold text-slate-500 dark:text-slate-500 tracking-wide leading-relaxed">{desc}</p>
...
function InputField({ label, type = "text", ...props }) {
  return (
    <div className="space-y-3 group/field">
      <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1 group-focus-within/field:text-primary transition-colors">{label}</label>
      <input
        type={type}
        className="w-full px-6 py-5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[20px] text-xs font-bold text-slate-900 dark:text-white focus:bg-white dark:focus:bg-white/10 focus:ring-8 ring-primary/5 focus:border-primary outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-700 shadow-sm"
        {...props}
      />
    </div>
  );
