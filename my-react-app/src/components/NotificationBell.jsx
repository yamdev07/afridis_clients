  ...
  return (
    <div className="relative" ref={bellRef}>
      ...
      >
        ...
        {count > 0 && (
          <span className="absolute -top-1 -right-1 flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-red-500 border-2 border-white dark:border-slate-950 px-1 text-[10px] font-black text-white shadow-lg">
            {count > 9 ? "9+" : count}
          </span>
        )}
      </motion.button>
      ...
      <AnimatePresence>
        {opened && (
          <motion.div
            ...
            className="absolute right-0 mt-4 w-[420px] rounded-[28px] bg-white dark:bg-slate-900 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] dark:shadow-none overflow-hidden z-[100] border border-slate-200 dark:border-white/10"
          >
            {/* Header */}
            <div className="px-8 py-6 bg-slate-50/50 dark:bg-white/5 border-b border-slate-100 dark:border-white/10 flex items-center justify-between">
              <div>
                <h3 className="text-base font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                  <Sparkles size={16} className="text-primary" /> Notifications
                </h3>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest mt-1">{count} messages non lus</p>
              </div>
              <button
            ...
            <div className="max-h-[480px] overflow-y-auto custom-scrollbar">
              ...
              {error && (
                <div className="p-10 text-center">
                  <div className="w-12 h-12 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
              ...
              {!loading && items.length === 0 && (
                <div className="p-24 flex flex-col items-center justify-center gap-6 text-center">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Silence absolu</p>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium tracking-tight">Aucune alerte capturée pour le moment.</p>
                  </div>
                </div>
              )}
                ...
                {items.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex gap-5">
                      <div className={`mt-1 h-12 w-12 rounded-[18px] flex items-center justify-center flex-shrink-0 border transition-transform group-hover:scale-110 duration-300 ${n.type === 'alert' ? 'bg-red-50 dark:bg-red-500/10 text-red-500 border-red-500/10' : 'bg-primary/5 dark:bg-primary/20 text-primary border-primary/10'
                        }`}>
                        {n.type === 'alert' ? <AlertTriangle size={24} strokeWidth={2.5} /> : <Info size={24} strokeWidth={2.5} />}
                      </div>
                      <div className="flex-grow min-w-0">
                        <div className="flex justify-between items-start gap-4">
                          <p className="text-[13px] font-black text-slate-900 dark:text-white truncate tracking-tight">{n.title}</p>
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-300 dark:text-slate-700 hover:text-red-500 dark:hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                            >
            ...
            {/* Footer */}
            <div className="px-8 py-5 bg-slate-50 dark:bg-white/5 border-t border-slate-100 dark:border-white/10 flex items-center justify-center">
              <button className="flex items-center gap-2 text-[10px] font-black text-slate-500 dark:text-slate-500 hover:text-primary transition-all uppercase tracking-[0.2em] group">
